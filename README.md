...
Os cinco JPGs da raiz são originais de referência e não devem ser apagados. Há três imagens distintas e duas cópias duplicadas. `.img/Marble_sculpture_loop_animation_20260917161751.mp4` é a fonte do vídeo em loop do Hero, e os quatro MP3 em `.img/` (Vinyl Crackles, Quiet Reflection, Quiet Contemplation e sua variação) são a fonte da playlist ambiente. `scripts/prepare-assets.mjs` produz os arquivos utilizados pelo site (imagens em `public/images/`, vídeo recodificado em `public/videos/`, faixas de áudio em `public/audio/`); alterações nos derivados devem ser reproduzíveis pelo script. A recodificação do vídeo usa `ffmpeg-static` (binário do FFmpeg instalado via npm, sem dependência do sistema), remove o áudio e nunca faz upscale além da largura nativa da fonte, cortando o peso original para uma fração sem perda perceptível. O vídeo só é reproduzido automaticamente quando o visitante não pede `prefers-reduced-motion: reduce`; caso contrário, permanece parado no pôster (`public/images/hero-poster.jpg`).

O player ambiente (`src/components/AmbientPlayer.astro`, incluído em todas as páginas via `BaseLayout`) nunca reproduz áudio automaticamente — nenhum navegador permite autoplay com som, e a Diretriz 1.4.2 do WCAG exige controle do visitante sobre qualquer áudio. O botão flutuante no canto inferior direito inicia a playlist só quando clicado, sempre por uma faixa aleatória, avançando em loop contínuo pelas quatro faixas (recodificadas em `public/audio/`: `eco-do-tempo.mp3` mono a 64kbps para o ruído de vinil, e as três faixas melódicas em estéreo a 96kbps).
