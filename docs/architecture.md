...
Em 2026-09-17, um vídeo de referência substituiu a imagem estática do emblema no Hero; nesse mesmo dia o Hero também passou a usar o vídeo como fundo full-bleed (min-height: 100svh), com um overlay em degradê para legibilidade do texto, em vez da moldura circular original. A fonte atual é `.img/Marble_sculpture_loop_animation_20260917161751.mp4` (720p, 10s, ~4,3MB), substituindo uma primeira versão (`Marble_face_loop_animation`, 1080p) usada inicialmente com a moldura circular. `scripts/prepare-assets.mjs` usa `ffmpeg-static` para recodificar o clipe: remove o áudio (o vídeo é decorativo e sempre mudo), nunca faz upscale além da largura nativa da fonte (`scale='min(1600,iw)':-2`) e recomprime em H.264/CRF 28 — o WebM/VP9 testado no mesmo ajuste ficou maior, então não foi adotado. O primeiro frame vira o pôster (`public/images/hero-poster.jpg`), exibido enquanto o vídeo carrega e usado como estado final para quem prefere menos movimento.
